...
| --- | --- |
| Release | `v0.2.4` |
| Artifact | `ReadArc-0.2.4-macOS-arm64.dmg` |
...
```bash
./script/release_github.sh --version 0.2.4 --ad-hoc --skip-notary --format dmg
```

Publish a GitHub Release with the ad-hoc Apple Silicon DMG:

```bash
./script/release_github.sh --version 0.2.4 --publish --ready --ad-hoc --skip-notary --format dmg
```
...
./script/release_github.sh --version 0.2.4 --publish --ready --format dmg
```
