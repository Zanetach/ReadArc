...
ICON_SOURCE="$ROOT_DIR/design/assets/pdf-reader-logo.svg"
...
if [[ "$BUILD_CONFIGURATION" == "release" ]]; then
  ...
  BUILD_BINARY="$(swift build -c release --build-system native --show-bin-path)/$PRODUCT_NAME"
else
  swift build --build-system native
  BUILD_BINARY="$(swift build --build-system native --show-bin-path)/$PRODUCT_NAME"
fi
